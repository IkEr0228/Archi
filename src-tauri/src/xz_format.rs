//! Single-stream XZ open/list/extract (not tar.xz).
...
fn xz_error(code: &str, message: impl Into<String>) -> CommandError {
...
/// Logical entry name for a single xz payload (no embedded filename in format).
pub fn xz_entry_name(archive_path: &Path) -> Result<String, CommandError> {
    let file_name = archive_path
        .file_name()
        .map(|s| s.to_string_lossy().into_owned())
        .unwrap_or_else(|| "data".into());
    let lower = file_name.to_ascii_lowercase();
    let stem = if lower.ends_with(".txz") {
...
pub fn open_xz(path: &Path) -> Result<ArchiveInfo, CommandError> {
    if !path.is_file() {
        return Err(xz_error("not_found", "File not found or is not a file."));
    }
    let name = xz_entry_name(path)?;
    let on_disk = fs::metadata(path).map(|m| m.len()).unwrap_or(0);

    // Stream size only (no full payload in RAM) — potato-friendly open of large .xz.
    let file = File::open(path)
        .map_err(|error| xz_error("invalid_archive", format!("Cannot open xz: {error}")))?;
    let mut decoder = XzDecoder::new(file);
    let mut buffer = [0_u8; BUFFER_SIZE];
    let mut size = 0_u64;
    loop {
        let n = decoder.read(&mut buffer).map_err(|error| {
            xz_error("invalid_archive", format!("Cannot decompress xz: {error}"))
        })?;
        if n == 0 {
            break;
        }
...
pub fn extract_xz(
    ...
    if operation_id.is_empty() {
        return Err(xz_error("invalid_operation", "Operation ID is empty."));
    }
    if !path.is_file() {
        return Err(xz_error("not_found", "Source archive does not exist."));
    }
    ...
    if let Some(sel) = selected_paths {
        ...
        if !ok {
            return Err(xz_error(
                "empty_selection",
                "Selection does not match the xz entry.",
            ));
        }
    }
            ...
            Ok(meta) => {
                ...
                match decision {
                    ...
                    ConflictDecision::Cancel => {
                        return Err(xz_error("cancelled", "Archive extraction was cancelled."))
                    }
    ...
    if skipped == 0 {
        ...
        #[cfg(windows)]
        {
            ...
            let file_name = write_to
                .file_name()
                .ok_or_else(|| xz_error("invalid_destination", "Destination has no file name."))?;
            ...
            let out = parent
                ...
                .map_err(|error| {
                    xz_error("write_failed", format!("Cannot create temp file: {error}"))
                })?;
            {
                ...
                loop {
                    ...
                    writer.write_all(&buffer[..n]).map_err(|error| {
                        xz_error("write_failed", format!("Cannot write file: {error}"))
                    })?;
                ...
                writer.flush().map_err(|error| {
                    xz_error("write_failed", format!("Cannot flush file: {error}"))
                })?;
            }
            drop(out);
            let created_file = created
                .get(created_index)
                .ok_or_else(|| xz_error("write_failed", "Missing created temp file handle."))?;
        ...
        #[cfg(not(windows))]
        {
            if let Some(parent) = write_to.parent() {
                fs::create_dir_all(parent).map_err(|error| {
                    xz_error("write_failed", format!("Cannot create parents: {error}"))
                })?;
            }
            let mut out = fs::File::create(&write_to).map_err(|error| {
                xz_error("write_failed", format!("Cannot create file: {error}"))
            })?;
            loop {
                if cancelled.load(Ordering::Relaxed) {
                    return Err(xz_error("cancelled", "Archive extraction was cancelled."));
